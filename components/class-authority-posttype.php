...
class Authority_Posttype {
	...
	{
		static $taxonomy_objects;
		
		if ( $support )
		{
			$taxonomy_objects = get_taxonomies( array( 'public' => true ), 'objects' );

			$purge = array_diff( array_keys( $taxonomy_objects ), $support );

			foreach( $purge as $remove )
			{
				unset( $taxonomy_objects[ $remove ] );
			}//end foreach

			// sort taxonomies by the singular name
			uasort( $taxonomy_objects, array( $this , '_sort_taxonomies' ));
		}//end if
